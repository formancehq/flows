...
if [[ -n "${FCTL_SDK_ROOT:-}" ]]; then
  ...
  # CI has no sibling fctl checkout. Use an external cache so validation cannot
  # dirty the repository and project only the immutable lock commit.
  default_cache_root="${XDG_CACHE_HOME:-${HOME}/.cache}/formancehq/fctl-sdk"
  cache_directory="${FCTL_SDK_CACHE_DIR:-$default_cache_root/fctl-v2-poc.git}"
  readonly default_cache_root cache_directory
