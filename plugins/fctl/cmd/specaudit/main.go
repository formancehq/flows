// Command specaudit regenerates the deterministic Flows (orchestration)
// operation inventory from this repository's merged OpenAPI document.
//
...
func main() {
	spec := flag.String("spec", filepath.Join("..", "..", "openapi.yaml"), "path to the merged orchestration OpenAPI document")
...
func run(spec, out string, check bool) error {
	...
	if !check {
		...
		fmt.Fprintf(os.Stderr,
			"operations=%d v1=%d v2=%d deprecated=%d baseline=%d mapped=%d excluded=%d blocked=%d\n",
			t.SpecOperations, t.V1Operations, t.V2Operations, t.DeprecatedOperations,
			t.BaselineCommands, t.BaselineMapped, t.BaselineExcluded, t.Blocked)
	}
	return nil
}
