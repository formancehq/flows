...
// connectors API only exists from payments major release v3 onward, so a caller that wants
// it has to know which release it's talking to first - see resolveConnectorID.
...
func (a Activities) getPaymentsVersion(ctx context.Context) (*paymentsVersion, error) {
	...
	resp, err := a.client.Payments.V1.GetServerInfoPayments(ctx)
	if err != nil {
		return nil, fmt.Errorf("fetching payments server info: %w", err)
	}
	if resp.ServerInfo == nil || resp.ServerInfo.Version == nil {
		return nil, fmt.Errorf("payments server info response missing version")
	}

	v := computePaymentsVersion(*resp.ServerInfo.Version)
